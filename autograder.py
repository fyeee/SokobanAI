...
if __name__ == "__main__":
    if test_time_astar:
        ...
        time_bound = 3
        p = multiprocessing.Process(target=anytime_weighted_astar, name="Anytime A star",
                                    args=(PROBLEMS[19], heur_alternate, 1, time_bound))
        p.start()
        p.join(3.1)
        if p.is_alive():
    ...
    if test_time_gbfs:
        ...
        time_bound = 3
        p = multiprocessing.Process(target=anytime_gbfs, name="Anytime GBFS",
                                    args=(PROBLEMS[19], heur_alternate, time_bound))
        p.start()
        p.join(3.1)
        if p.is_alive():
        ...
        for i in range(0, len(PROBLEMS)):
            ...
            if final:
                ...
                if final.gval <= len_benchmark[index] or len_benchmark[index] == -99:
                    ...
                    # print(index, final.gval, len_benchmark[index])
                    result.append((index, final.gval, len_benchmark[index]))
                solved += 1
            else:
